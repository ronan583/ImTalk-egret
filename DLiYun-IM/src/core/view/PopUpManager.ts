...
module PopUpManager {
    ...
    */
	export function addPopUp(panel, dark: boolean = false, popUpWidth: number = 0, popUpHeight: number = 0, effectType: number = 0, isAlert: boolean = false): void {
		...
		panel.width = egret.MainContext.instance.stage.stageWidth;
		panel.height = egret.MainContext.instance.stage.stageHeight;
    ...
    */
	export function removePopUp(panel, effectType: number = 0): void {
		...
		if (panel["title"] != null) {
			egret.Tween.get(panel["title"]).to({ alpha: 0, scaleX: 0.5, scaleY: 0.5, x: panel["title"].x + panel["title"].width / 2, y: panel["title"].y + panel["title"].height / 2 }, 80).call(function(){
				remove();
			});
		} else {
			remove();
